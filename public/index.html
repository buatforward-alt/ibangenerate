...
<head>
    ...
    <style>
        ...
        body {
            ...
            /* PERBAIKAN: Menggunakan background.jpg sesuai nama file kamu */
...
<div class="container">
    ...
    <div id="bc-container" class="card hidden" style="border: 1px solid var(--primary); background: rgba(0, 210, 255, 0.1);">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <b style="color: var(--primary); font-size: 12px;">📢 PENGUMUMAN ADMIN</b>
    ...
    <div id="auth-panel" class="card hidden">
        <h2 id="auth-title" style="color: var(--primary); margin-top: 0; font-size: 18px;">🔑 LOGIN ACCESS</h2>
    ...
    <div id="main-content" class="hidden">
        ...
        <div class="card">
            <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                <div style="display: flex; align-items: center; gap: 15px;">
                    <div id="profile-icon" style="font-size: 28px; background: rgba(0,0,0,0.5); padding: 12px; border-radius: 50%; border: 1px solid #333;">👤</div>
                    <div>
        ...
        <div id="panel-user">
            <div class="card" id="gen-card" style="opacity: 0.2; pointer-events: none; transition: 0.3s;">
                <h2 style="color: var(--primary);">🌍 IDENTITY GENERATOR</h2>
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-top: 15px;">
                    <button onclick="generate('DE')">🇩🇪 GERMANY</button>
                    <button onclick="generate('FR')">🇫🇷 FRANCE</button>
                    <button onclick="generate('NL')">🇳🇱 NETHERLANDS</button>
                    <button onclick="generate('GB')">🇬🇧 UK</button>
                </div>
        ...
        <div id="panel-admin" class="hidden">
            ...
            <div class="card">
                <h2 style="color: var(--primary);">👥 REGISTERED USERS</h2>
                ...
                <button class="btn-red" onclick="resetAll()">🗑️ RESET ALL DATABASE</button>
            </div>
            
            <div class="card">
                <h2 style="color: var(--primary);">🎟️ ACTIVE VOUCHERS</h2>
                <div style="max-height: 200px; overflow-y: auto; background: #000; border-radius: 8px;">
                    <table>
...
<script>
    ...
    function startAuth(method) {
        ...
        document.getElementById('auth-title').innerText = `🔑 LOGIN ${method.toUpperCase()}`;
    }

    function toggleAuthMode() {
        isResetMode = false;
        authMode = authMode === 'login' ? 'signup' : 'login';
        document.getElementById('auth-title').innerText = authMode === 'login' ? '🔑 LOGIN ACCESS' : '📝 SIGN UP';
        ...
        if (authMode === 'login') {
            ...
            if (currentOTP) {
                ...
                try {
                    await axios.post(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
                        chat_id: tempId,
                        text: `🔐 *NINEPACMAN.ID VERIFICATION*\n\nKode OTP Anda: \`${currentOTP}\`\nJangan berikan kode ini kepada siapapun.`,
                        parse_mode: 'Markdown'
                    });
                    document.getElementById('login-fields').classList.add('hidden');
                    document.getElementById('otp-field').classList.remove('hidden');
    ...
    async function generate(cc) {
        ...
        resBox.innerText = "⏳ Generating Verified Data...";
        try {
    ...
    function syncStatus() {
        ...
        document.getElementById('profile-icon').innerText = (userId === ADMIN_ID ? '⚡' : '👤');
        ...
        if (u.isPremium && (userId === ADMIN_ID || Date.now() < u.expiry)) {
            document.getElementById('status-info').innerHTML = `STATUS: <span style="color:var(--primary)">PREMIUM ✅</span>`;
            ...
            document.getElementById('status-info').innerHTML = `STATUS: <span style="color:#ff4444">FREE ❌</span>`;
    ...
    async function sendBC() {
        ...
        if (BOT_TOKEN && chatid) {
            try {
                await axios.post(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, { chat_id: chatid, text: `📢 *NINEPACMAN.ID ANNOUNCEMENT*\n\n${msg}`, parse_mode: 'Markdown' });
